...
def optionwindow():
    ...
    text4 = font.render('Choose Backgroundnd colour:', True, green, blue)
...
while run:
    ...
    if snake_body[0][0] == food_x and snake_body[0][1] == food_y:
        food_x = 20*(random.randint(0, (width//20)))
        food_y = 20*(random.randint(0, (height//20)))
        snake_body.append([snake_body[-1][0],snake_body[-1][1]])
        pygame.display.update()

        #Check if snake and body overlap
    lstx=[] #list containing the x co-ordinates of the other body parts
    lsty=[] #list containing the y co-ordinates of the other body parts
    for j in range(1,len(snake_body)):
        lstx.append(snake_body[j][0])
        lsty.append(snake_body[j][1])
    if snake_body[0][0] in lstx:
        xcor=lstx.index(snake_body[0][0])
    if snake_body[0][1] in lsty:
        ycor=lsty.index(snake_body[0][1])
    if xcor==ycor:
        game_over


        # Check if snake hits the boundary of the screen in PEACE MODE
    #for i in range(0,len(snake_body)):
        #if snake_body[i][0]>1366:
         #   snake_body[i][0]=0
        #elif snake_body[i][0]<0:
         #   snake_body[i][0]=1366
        #if snake_body[i][1]<0:
         #   snake_body[i][1]=710
        #elif snake_body[i][1]>710:
         #   snake_body[i][1]=0            
